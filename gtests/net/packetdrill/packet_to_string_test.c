...
static void test_tcp_ipv4_packet_to_string(void)
{
	/* A TCP/IPv4 packet. */
	u8 data[] = {
		0x45, 0xff, 0x00, 0x3c, 0x00, 0x00, 0x00, 0x00,
		0xff, 0x06, 0x38, 0x12, 0xc0, 0x00, 0x02, 0x01,
	...
	printf("dump = '%s'\n", dump);
	expected =
		"[tos 255] . 1:1(0) ack 2202903899 win 257 "
	...
	expected =
		...
		"<sack 2202905347:2202906795,TS val 300 ecr 1623332896>"
		"\n"
		"0x0000: 45 ff 00 3c 00 00 00 00 ff 06 38 12 c0 00 02 01 " "\n"
...
static void test_tcp_ipv6_packet_to_string(void)
{
	/* A TCP/IPv6 packet. */
	u8 data[] = {
		0x63, 0x30, 0x00, 0x00, 0x00, 0x20, 0x06, 0xff,
	...
	printf("dump = '%s'\n", dump);
	expected =
		"[tos 51] S 0:0(0) win 32792 <mss 1000,sackOK,nop,nop,nop,wscale 7>";
	assert(strcmp(dump, expected) == 0);
	free(dump);
	...
	expected =
		"2001:db8::1:54242 > fd3d:fa7b:d17d::1:8080 "
		"[tos 51] S 0:0(0) win 32792 <mss 1000,sackOK,nop,nop,nop,wscale 7>\n"
		"0x0000: 63 30 00 00 00 20 06 ff 20 01 0d b8 00 00 00 00 " "\n"
